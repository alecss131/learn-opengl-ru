...
Для демонстрации диффузных карт мы будем использовать [изображение](container2.png) деревянного контейнера с железной рамкой:
...
Обновленные вершинные данные можно найти [здесь](src1.cpp). Теперь они включают в себя позиции вершин, векторы нормалей и текстурные координаты для каждой вершины куба. Давайте обновим вершинный шейдер, чтобы он мог принимать текстурные координаты, как вершинный атрибут и передавать их во фрагментный шейдер:
...
Вы можете найти полный исходный код приложения [здесь](src2.cpp).
...
Мы снова можем воспользоваться текстурной картой, только теперь для зеркальных бликов. Это значит, что нам нужно создать черно-белую \(или цветную, если хотите\) текстуру, которая определит силу блеска каждой части объекта. Вот пример [бликовой карты](container2_specular.png):
...
Вы можете найти полный исходный код приложения [здесь](src3.cpp).
...
2. Попробуйте инвертировать цвета бликовой карты во фрагментном шейдере, т.е. дерево должно блестеть, а железная рамка — нет \(заметьте, что трещины на рамке все еще дают зеркальное отражение, хотя и с меньшей интенсивностью\): [решение](exercise1.fs).
3. Попробуйте создать карту бликов из диффузной карты, которая использует не черно-белые цвета, и вы увидите, что результат не очень реалистичный. Вы можете использовать [эту цветную карту бликов](lighting_maps_specular_color.png), если не можете сделать ее сами. [Результат](lighting_maps_exercise3.png).
4. Вы также можете попробовать добавить к нашему кубу *карту эмиссии*, которая хранит значение свечения каждого фрагмента объекта. Значения эмиссии — это цвета, которые объект может *излучать*, как если бы он содержал источник света. Таким образом, объект может светиться независимо от условий освещения. Обычно карты свечения можно увидеть на тех игровых объектах, которые светятся \(например [глаза робота](shaders_enemy.jpg) или [полосы света на контейнере](emissive.png)\). Добавьте эту [текстуру](matrix.jpg) \(от creativesam\) на наш контейнер в качестве карты свечений, так, чтобы буквы излучали свет. [Решение](exercise2.cpp), [результат](lighting_maps_exercise4.png).
