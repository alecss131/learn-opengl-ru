...
[Здесь](skybox.zip) в хорошем разрешении можно скачать текстуру для скайбокса, которую будем использовать в уроке.
...
Поскольку без объекта куба нам не обойтись, то понадобятся новые VAO, VBO и массив вершин, который можно скачать [здесь](src1.cpp).
...
Код с применением этой оптимизации можно найти [здесь](src2.cpp).
...
Из-за наличия нормалей придется обновить и используемый [массив данных вершин](src3.cpp), а также обновить код, связанный с настройкой указателей атрибутов вершин. Также не забудьте устанавливать значение юниформа *cameraPos* в каждой итерации рендера.
...
Исходный код находится [здесь](src4.cpp).

Когда всему объекту назначен подобный спецэффект он выглядит как выполненный из сильно отражающего материала как полированная сталь или хром. Если загрузить модель нанокостюма из урока по [загрузке моделей](../../part%203/chapter%203/text.md), то весь костюм будет выглядеть цельнометаллическим:
...
Попробуйте добавить карту отражения в загрузчик моделей, созданный в соответствующем [уроке](../../part%203/chapter%2012/text.md). Данные модели нанокостюма, включая саму текстуру карты отражений, можно скачать [здесь](nanosuit_reflection.zip). Отмечу несколько моментов:
