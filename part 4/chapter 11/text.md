...
Исходник примера находится [здесь](src1.cpp).
...
Есть два основных метода создания буферов с мультисэмплингом для дальнейшего присоединения к кадровому буферу, сходных с уже разобранными примерами из соответствующего [урока](../../part%204/chapter%205/text.md): текстурные прикрепления и прикрепления типа рендербуфера.
...
Данная функция осуществляет перенос области-источника, заданной четырьмя координатами в экранном пространстве, в область-приемник, также заданную четырьмя экранными координатами. Напомню урок по [кадровым буферам](../../part%204/chapter%205/text.md): если мы привязываем объект буфера кадра к цели *GL_FRAMEBUFFER*, то неявно привязка осуществляется и к цели чтения из буфера кадра и к цели записи в буфер кадра. Для привязки к этим целям по отдельности использутся специальные идентификаторы целей: *GL_READ_FRAMEBUFFER* и *GL_DRAW_FRAMEBUFFER* соответственно.
...
Исходники примера находятся [здесь](src2.cpp).
...
Если мы добавим этот код к примерам постобработки из урока по [буферу кадра](../../part%204/chapter%205/text.md), то сможем применить все те эффекты к изображению сцены без зазубренных краев. Например, с эффектом размытия получится что-то подобное:
